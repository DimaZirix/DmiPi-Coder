...
You do not have to build anything to run the console: every [GitHub release](https://github.com/DimaZirix/DmiPi-Coder/releases) carries `agent-console-<version>-all.jar`, one jar with the engine and its libraries inside (§2). To embed the core, take `agent-core-<version>.jar` from the same release or build from source (§13). To build from source, from the repository root:

```bash
mvn -q install
```

This runs the tests and leaves the runnable console at `agent-console/target/agent-console-<version>-all.jar`. Pushes to `master` and tags run the same build on GitHub Actions; a tag `vX.Y.Z` also creates the GitHub release with the console jar and the `agent-core` library jar attached. The library is not published to any Maven repository.
...
**Getting the library.** `agent-core-<version>.jar` is attached to every release; it depends on `jackson-databind` 3.1.4 and `java-diff-utils` 4.17. It is not on Maven Central; `mvn -q install` from a checkout installs `com.dmipi:agent-core:<version>` into your local repository. The Java packages are `com.dmipi.coder.*`.
